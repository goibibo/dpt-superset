...
export const HYDRATE_EXPERIMMENT_DASHBOARD = 'HYDRATE_EXPERIMMENT_DASHBOARD';
...
export const hydrateExperimentDashboard =
  ...
  (dispatch, getState) => {
    ...
    charts.forEach(slice => {
      if (slice.variantSliceId) {
        const key = slice.slice_id;
        // ........custom_code
        const sliceKey = slice.variantSliceId;
        /**
         * @param sliceKey
         * @param slice.variantSliceId
         * @param slice.variantId
         * @param `${slice.slice_name}-${slice.variantId}`
         */
        // .........custom_code_end
        const formData = {
          ...slice.form_data,
        ...
        sliceIds.add(sliceKey);

        // if there are newly added slices from explore view, fill slices into 1 or more rows
        if (!chartIdToLayoutId[sliceKey] && layout[parentId]) {
          if (
            newSlicesContainerWidth === 0 ||
            newSlicesContainerWidth + GRID_DEFAULT_CHART_WIDTH >
              GRID_COLUMN_COUNT
          ...
          const count = (slicesFromExploreCount.get(slice.slice_id) ?? 0) + 1;
          chartHolder.id = `${CHART_TYPE}-explore-${sliceKey}-${count}`;
          slicesFromExploreCount.set(sliceKey, count);

          layout[chartHolder.id] = chartHolder;
          newSlicesContainer.children.push(chartHolder.id);
          chartIdToLayoutId[sliceKey] = chartHolder.id;
          newSlicesContainerWidth += GRID_DEFAULT_CHART_WIDTH;
        }
        ...
        if (slice.form_data.viz_type === 'filter_box') {
          ...
          const componentId = chartIdToLayoutId[key];
          const directPathToFilter = (
            layout[componentId].parents || []
          ).slice();
          directPathToFilter.push(componentId);
          dashboardFilters[key] = {
            ...dashboardFilter,
        ...
        if (layoutId && layout[layoutId]) {
          ...
          layout[layoutId].meta.variantIndex = slice.variantIndex;
          layout[layoutId].meta.width = GRID_DEFAULT_CHART_WIDTH;
        }
        // ........custom_code_end
      } else {
        const key = slice.slice_id;
