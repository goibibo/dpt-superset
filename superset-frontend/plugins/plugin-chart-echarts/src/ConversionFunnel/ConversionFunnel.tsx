...
export default function EchartsConversionFunnel(
  ...
  const chartOptions: any = echartOptions;
  const formatter = (data: any) => chartOptions?.tooltip?.formatter(data);
  //   const parentChart = useSelector<RootState, ChartsState>(
  //     state => state.charts[formData.sliceId],
  //   );
  //   const parentChart = null;
  //   const [totalExperimentData, setTotalExperimentData] = useState(null);
  const metricsDataList = chartOptions?.series[0].data;
  ...
  const recalculate = (metricList: any) => {
    try {
      const metricListData = metricList;
        ...
        if (metricListData[i].display) {
          ...
          if (newList.length) {
            const changePercentage = (
              (metricListData[i].value / newList[newList.length - 1].value) *
              100
            ).toFixed(2);
            metricListData[i] = {
              ...metricListData[i],
